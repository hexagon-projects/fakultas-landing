  ...
  <div class="w-full h-fit flex justify-center relative z-50">
    ...
    <nav :class="[
      ...
      <div class="flex items-center gap-3 md:gap-4 z-1 cursor-pointer" @click="navigateToHome">
        <img :src="logo" alt="Logo Universitas Pasundan" class="w-10 h-10 md:w-12 md:h-12 object-cover" loading="lazy"
          width="48" height="48" />
        <div class="w-[85%]">
      ...
      <img :src="union" alt="" class="absolute top-0 left-0 h-full w-[23%] object-fit -z-10" loading="lazy"
        aria-hidden="true" />

      <div
        :class="['hidden xl:w-9/12 md:flex items-center xl:justify-between gap-4 xl:gap-8 font-medium', navbarColors]">
        <RouterLink v-for="(item, index) in menuItems" :key="index" :to="item.path" :aria-label="item.name"
          ...
          {{ item.name }}
          <motion.div v-if="route.path === item.path" layoutId="activeNavItem" initial="{ scaleY: 0 }"
