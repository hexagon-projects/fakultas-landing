<template>
  <button ref="buttonRef" :class="cn(
    'group relative w-auto cursor-pointer overflow-hidden rounded-full border text-xs lg:text-sm font-bold text-center transition-all duration-300',
    props.bgColor || 'bg-fakultas',
    props.border || 'border-[3px]',
    props.borderColor || 'border-outline/50',
    props.textColor || 'text-netral',
    props.borderHover ? `hover:${props.borderHover}` : 'hover:border-fakultas',
