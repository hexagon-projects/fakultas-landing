...
<template>
  <div class="py-12">
    <transition name="fade" mode="out-in">
      <div
        :key="currentIndex"
        class="flex flex-col md:flex-row gap-10 lg:gap-20"
        v-if="currentTestimoni"
      >
        <div class="w-full md:w-1/2">
          <div class="bg-gray-200 w-full h-80 rounded-[32px] overflow-hidden flex items-center justify-center">
            <img
        ...
        <div class="w-full md:w-1/2 flex flex-col gap-5 justify-center">
          <p class="text-sm md:text-base" v-html="currentTestimoni.description" />
          <div>
            <p class="font-semibold">{{ currentTestimoni.name }}</p>
            <p>{{ currentTestimoni.title }}</p>
          </div>
        </div>
      </div>
    </transition>
