  ...
  <MainLayout>
    ...
    <SectionLayout>
      <div class="flex flex-col lg:flex-row md:justify-between relative">
        <div class="lg:w-[45%] w-full h-full lg:sticky lg:top-40">
          <img
            :src="getImageUrl(fakultasStore.fakultas?.image3 || SejarahImg)"
            alt=""
    ...
    <SectionLayout>
      <div class="grid gap-2 lg:gap-5">
        ...
        <TitleSection
          v-if="fakultasStore?.fakultas?.name"
          :text="`<span class='text-colorPrimary'>${fakultasStore?.fakultas?.name}</span> Unpas`"
          :html="true"
        >
        </TitleSection>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-8">
        <DosenCard
          v-for="dosen in teamStore.teams.slice(0, 12)"
          :key="dosen.id"
          :name="dosen.name"
          :title="dosen.title"
    ...
    <TestimonialsSection
      v-if="berandaStore.testimoniData"
      :testimoni="berandaStore.testimoniData"
      :is-loading="false"
    />
    <CTASection />
  </MainLayout>
